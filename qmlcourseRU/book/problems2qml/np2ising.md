...
Для этой задачи было разработано немало [эвристических алгоритмов поиска](https://journals.aps.org/pre/abstract/10.1103/PhysRevE.90.012811). В этой части сосредоточимся на формулировке как задачи `QUBO`.

### $k = 2$ сообществ

При это будем использовать двоичные спиновые переменные $s_i \in \{-1,1\}$ для кодирования к какому именно сообществу принадлежит вершина $i$. $\frac{1 + s_is_j}{2}$ будет равняться $1$, если $i$ и $j$ принадлежат одному сообществу и $0$ в противоположном. Модулярность при этом будет следующей
...
При $s_i = 2 x_i - 1$ между спиновыми $s_i \in \{-1,1\}$ и битовыми $x_i \in \{0, 1\}$ переменными соответственно, а также $\sum_{i,j} B_{i,j} = 0$, максимизация модулярности {eq}`eqn:3` может быть эквивалентна выражению задачи минимизации в форме `QUBO` с гамильтонианом $H = - \frac{1}{m} x^T B x$ и с `QUBO` матрицей $Q = - \frac{B}{m}$.

### Множественные сообщества ($k > 2$)

Чтобы сформулировать задачу обнаружения $k$ сообществ в канонической форме `QUBO`, сначала одновременно кодируем двоичные переменные $x_i$, а затем строим `QUBO`-гамильтониан.
...
Представляя обобщенную матрицу модулярности $Β$ размером $kN \times kN$ и блочно-диагональной формы с $B$ вдоль диагонали {eq}`eqn:5`, можем переписать задачу обнаружения $k$-сообщества как задачу двоичной минимизации в каноническом формате `QUBO` (где исходные многоклассовые переменные вложены в большее число двоичных переменных)
...
с положительным предварительным коэффициентом $P > 0$, обеспечивающим выполнение ограничений. Чтобы сформулировать штрафной член в {eq}`eqn:6` в `QUBO`-гамильтониан $H_P = x^T Q_P x$, перенумеруем двоичный вектор решений, используя один подстрочный индекс от $1$ до $kN$, то есть $x = (x_{1,1}, x_{2,1}, ... , x_{N,1}, ... , x_{N,k}) = (x_1, x_2, ... , x_N , ... , x_{kN} )$. Тогда штрафной член {eq}`eqn:6` может быть переписан как
...
Объединив модульности в уравнении {eq}`eqn:5` и штрафной гамильтониан в уравнении {eq}`eqn:7`, получаем окончательный `QUBO`-гамильтониан для общего обнаружения сообщества с $k$ классами
