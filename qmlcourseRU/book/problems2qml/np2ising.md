...
При поиске разбиения графа на $k = 2$ сообществ, то можем использовать двоичные спиновые переменные $s_i \in \{-1, 1\}$ для кодирования к какому именно сообществу принадлежит вершина $i$. $\frac{1 + s_is_j}{2}$ будет равняться $1$, если $i$ и $j$ принадлежат одному сообществу и $0$ в противоположном. Модулярность при этом будет следующей
...
При $s_i = 2 x_i - 1$ между спиновыми $s_i \in \{-1, 1\}$ и битовыми $x_i \in \{0, 1\}$ переменными соответственно, а также $\sum_{i,j} B_{i,j} = 0$ может быть эквивалентно выражению задачи минимизации в форме `QUBO` с гамильтонианом $H = - \frac{1}{m} x^T B x$ и с `QUBO` матрицей $Q = \frac{-B}{m}$. Подобные матрицы могут быть легко переданы в квантовый отжиг, тому же D-Wave, который попытается найти оптимальную битовую строку $x$, что в свою очередь кодирует решение проблемы оптимизации.
