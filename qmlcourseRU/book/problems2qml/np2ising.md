...
При $s_i = 2 x_i - 1$ между спиновыми $s_i \in \{-1, 1\}$ и битовыми $x_i \in \{0, 1\}$ переменными соответственно, а также $\sum_{i,j} B_{i,j} = 0$, максимизация модулярности может быть эквивалентна выражению задачи минимизации в форме `QUBO` с гамильтонианом $H = - \frac{1}{m} x^T B x$ и с `QUBO` матрицей $Q = \frac{-B}{m}$.

Используем схему однократного кодирования, в которой задаем $x_{i,c} = 1$, если вершина $i$ принадлежит к сообществу $c$, и $x_{i,c} = 0$ в противном случае, то есть,
...
Тогда потребуется $k$ переменных на логическую вершину и размер двоичного вектора решений $x$ увеличится с вектора длинны $N$ для случая $k = 2$ до $k \times N$ для $k$-сообществ.
