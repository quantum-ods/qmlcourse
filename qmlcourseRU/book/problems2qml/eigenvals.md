...
```{note}
Замечание -- в классическом **VMC** мы обычно работаем при нулевой температуре. Хотя в общем случае, температура оказывает значительное влияние на то, в каком состоянии находится физическая система.
```
...
дает нам распределение вероятностей, а значит мы можем из него семплировать используя методы Монте-Карло. Это очень похоже на то, как мы семплировали из распределения Больцмана в [классическом методе Монте-Карло](../problemsblock/copt.html#id13). Вопрос лишь в том, как представить волновую функцию $\ket{\Psi}$? В этом нам помогут так называемые _trial wave functions_ -- параметризированные функции от $X$. В этом случае мы меняем, или _варьируем_ параметры _trial wave function_ в процессе:

- Сэмплируем из $\frac{|\Psi(X)|^2}{\int |\Psi(X)|^2 dX}$ конфигурации.
- Обновляем параметризацию _trial function_ так, чтобы минимизировать энергию.

Повторяем до сходимости. Ну а дальше мы посмотрим на некоторые примеры _trial wave functions_.

#### Jastrow Function

Если у нас есть задача из $N$ квантовых частиц, каждая из которых описывается координатой или радиус вектором, то мы можем построить _trial wave function_ в виде суммы попарных функций двухчастичных взаимодействий:

$$
\Psi(X) = e^{-\sum_{i,j} u(r_i, r_j)},
$$

где $r_i, r_j$ -- это радиус-векторы частиц, а $u(r_i, r_j)$ -- симметричная функция, описывающая двухчастичное взаимодействия. Такая функция называется **Jastrow function** {cite}`jastrow1955many`. В этом случае, в процессе работы нашего **VMC** мы будем просто варьировать радиус-векторы частиц также, как мы варьировали вершины графа в [обычном отжиге](../problemsblock/copt.html#id13), когда решали задачу комбинаторной оптимизации. Только теперь у нас есть еще и параметризация обменных взаимодействий, которую мы "варьируем".

#### Hartree-Fock (SCF)

Для задач квантовой химии, когда мы работаем с фермионами, существует вид _trial wave function_ на основе Слэтеровского детерминанта, про который мы говорили в [продвинутой лекции по квантовой химии](../problemsblock/quantchemadvancedscf.html#id13):
...
На самом дальше нас будут интересовать как раз модели Изинга и спины, а не частицы в пространстве или обритали из вторичного квантования. Для спинов мы можем записать Jastrow function следующим образом:
...
Так как наша модель относительно небольшая по числу частиц, то мы сразу можем получить точное решение методом Ланкзоша.

```{code-cell} ipython3
exact = nk.exact.lanczos_ed(op)[0]
```

Создадим модель на основе Jastrow и наш **VMC**:
...
```{note}
Мы оставляем изучение документации библиотеки `NetKet` читателю, так как объяснение абстракций графа и гильбертова пространства, а также использование метода _stochstic reconfiguration_ для вычисления градиентов выходит за рамки нашей лекции. Документаци представлена на [сайте NetKet](https://www.netket.org).
```
...
Еще более интересный подход к выбору _trial wave function_ -- это использование в качестве $\Psi(X)$ нейронной сети {cite}`carleo2017nqs`. Мы немного [касались этой темы](qmlkindsblock/qmlkinds.html#nqs), когда говорили о видах квантового машинного обучения. Хороший вариант, это использовать, например, полносвязную сеть -- ограниченную машину Больцмана:
...
Преимущества использования нейронной сети трудно показать на таком небольшом примере с моделью Изинга и 10-ю спинами, но они полностью раскрываются, если нам надо анализировать более сложные модели.
...
К сожалению, у метода **VMC** есть свои проблемы. Это относительно плохая масштабируемость -- при росте размерности проблемы для того, чтобы подобрать реально хорошую аппроксимацию нам нужно всне больше итераций и все больше сэмплов на каждой из них. Также у **VMC** есть ряд фундмаентальных проблем, например, так называемая _sign problem_ {cite}`loh1990sign`, правда их рассмотрение выходит за рамки этого курса.

## Заключение

В этой лекции мы рассмотрели известные подходы к решению задачи о минимальном собственном значении на классическом компьютере. Как мы увидели, все эти методы не могут быть масштабированы на реально большие операторы. Так что нам для решения этих проблем действительно нужен квантовый компьютер.
