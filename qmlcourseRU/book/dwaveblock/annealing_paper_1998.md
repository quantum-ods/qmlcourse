...
Третье условие в {eq}`eqn:L_trans_matr_elem` означает, что мы рассматриваем только перевороты одиночных спинов (*single-spin flip*) в качестве допустимых переходов между состояниями системы.
