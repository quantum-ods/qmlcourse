...
соответствуют операторам создания и разрушения.
...
решетке. В соответствии с принципом Паули, на каждой вершине может быть максимум два электрона (один со спином вверх и
один со спином вниз).
...
где \langle i,j \rangle означает что $i$ и $j$ индексы соседних вершин решетки, $\sigma$ обозначает спин электрона, а
h.c. (hermtian conjugate) обозначает слагаемое, эрмитово сопряжённое написанному.

Оператор $ c^\dagger_{i, \sigma}c_{j, \sigma}$ уничтожает электрон на вершине $j$ и создает электрон с таким же спином
на вершине $i$, то есть, фактически, перемещает электрон с вершины $j$ на вершину $i$.

```{note}
Эрмитово сопряжённый оператор $ \left(c^\dagger_{i, \sigma}c_{j, \sigma}\right)^\dagger = c^\dagger_{j, \sigma}c_{i, \sigma}$  перемещает электрон с вершины $i$ на вершину $j$. 
```

Так как этот член квадратичный, мы можем точно решить гамильтониан содержащий только его.

Второй член содержит отталкивающее взаимодействие двух электронов на одной вершине, соответствующее отталкиванию двух
зарядов:
...
Для того чтобы описать соответствующий спиновый Гамильтониан, каждой вершине мы сопоставим два кубита (один для каждого
