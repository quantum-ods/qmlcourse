...
Перед тем как приступить, рекомендуется освежить [материал](classicsvm) по классическому методу опорных векторов, в частности, нужно понимать, как работает ядровый трюк.
...
$$
\braket{U(x)|U(x)} = 1$ и $\braket{U(x_1)|U(x_2)} = \braket{U(x_2)|U(x_1)}
$$
