...
В прошлых лекция активно [рассказывалось](dwave) о квантовых аннилерах -- аналоговых устройствах, реализующих поиск основного состояния системы. Выпишем еще раз [гамильтониан](../dwaveblock/dwave.html#id4), который там используется:
...
И в лекции о `D-Wave` указывалось на главную проблему -- риск перехода системы из основного состояния в возбужденное при недостаточно медленном изменении параметров $A$ и $B$:
...
У отжига "в железе" есть существенная проблема -- нам трудно правильным образом подбирать расписание отжига. Но есть и другая идея! Изначально квантовые компьютеры создавались для симуляции квантовой динамики. Воспользуемся техникой симуляции, которая называется _trotterization_.

### Trotterization и симуляция аннилера

В данном случае наша цель получить выражение для финального состояния $\Psi$, которое будет отвечать решению квантового аннилера. Финальное состояние есть решение уравнения Шредингера:
...
Для решение проблемы "нерешаемости" уравнение Шредингера перейдем от непрерывного времени $t$ и зависимостей $A(t)$ и $B(t)$ к $N$ дискретных моментов времени $t_1, t2_2, ..., t_n$. Тогда можно заменить расписание отжига в виде непрерывных коэффициентов на набор дискретных коэффициентов, каждый из которых отвечает своему моменту времени:
...
А это уже хорошо знакомая задача оптимизации результата измерения состояния, заданного при помощи некоторой [**VQC**](vqc), которая параметризированая набором действительных чисел $\gamma_1, ..., \gamma_N, beta_1, ..., \beta_N$. И эта задача решается хорошо уже знакомыми [градиентными методами](gradients).
