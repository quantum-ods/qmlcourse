...
Функция len() для словаря будет возвращать количество пар `ключ`: `значение`, котороое в нем содержится:
```{code-cell} ipython3
empty_dict = {}  
...
Не забудьте, когда в функции передается просто название переменнной со словарем, работа идет только над множеством ключей.
...
```
![counter указывает на 100](https://www.pythontutorial.net/wp-content/uploads/2020/10/Python-Immutable-Example-1.png)
...
![counter указывает на 200](https://www.pythontutorial.net/wp-content/uploads/2020/10/Python-Immutable-Example-2.png)

Из интересного: Python [заранее создает объекты](https://docs.python.org/3/c-api/long.html#c.PyLong_FromLong) для чисел от -5 до 256, поэтому для переменных со значением из этого диапазона бедутся заранее готовые ссылки
...
![list по адресу](https://www.pythontutorial.net/wp-content/uploads/2020/10/Python-Mutable-Example.png)

После добавления еще одного, адрес `ratings` не изменился, поменялось лишь внутреннее состояние:

![list по адресу, но с 4](https://www.pythontutorial.net/wp-content/uploads/2020/10/Python-Mutable-Example-2.png)


## Что мы узнали из лекции
- новый тип данных - **словарь**!
- как создавать словарь, добавлять, доставать или удалять значения
- как работать отдельно с ключами, значениями или со всем сразу
- что можно и нельзя хранить в словаре
