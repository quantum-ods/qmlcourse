...
## Что такое `dict`
`dict` (от английского _"dictionary"_, словарь) -- еще один тип данных в Python. Словари хранят пары `ключ`: `значение`. Это значит, что в списках можно достать элемент, указав на его позицию в виде целого числа, а в словарях -- на `ключ`. `Dict` -- неупорядоченный тип данных, поэтому достать элемент по номеру не получится, но отображение содержимого будет в порядке добавления элементов. Ключи должны быть уникальными, чтобы всегда можно было быстро найти одно единственно верное соответствующее ему `значение`.

В некоторых языках программирования можно встретить ассоциативные массивы -- полную аналогию `dict`. В база данных используется такая же логика: в таблице можно установить первичный ключ, который уникально идентифицирует запись. Самый простой пример ключа -- порядковый номер объекта.

### Создание словаря

Использовать словарь стоит тогда, когда нужно обращаться к объектам по известным ключам. Один из способов определения словаря -- это указание пар `ключ`: `значение` через запятую внутри фигурных скобок `{}`. Напоминает `set`, правда? `{}` позволяет создать пустой словарь, но не пустое множество.

Например, вы решили упростить себе жизнь и больше не запоминать дни рождения коллег. Лучше хранить их в одном месте:
...
Чтобы получить значение по ключу, нужно обратиться к переменной, содержащей словарь, и указать ключ в квадратных скобках `[]`:

```{code-cell} ipython3
dates["Кунг Фьюри"]
```

Если указать неверный ключ в `[]`, Python будет ругаться: выбросит ошибку `KeyError` и перестанет выполнять код. Посмотрим, как можно избежать этого.
...
print(dates)
```
Если ключ уже был в словаре, то значение по нему изменится на новое, а старое будет удалено. Указание нового ключа со значением добавляет пару в словарь.

## Основные методы словаря

### Проверка вхождения и `get()`
Ранее мы говорили, что обращение к несуществующему ключу приводит к ошибке -- пришло время посмотреть пару способов борьбы с ней.
...
Вторым аргументом метода `get()` можно указать значение, которое должно возвращаться вместо None, если ключ не был найден:
...
- при помощи инструкции del (от английского _"delete"_) можно удалить пару `ключ`: `значение`. Удаление ключа эквивалентно удалению пары `ключ`: `значение`, так как мы теряем возможность найти то самое `значение`. В общем виде:
...
Предположим, коллега из самого первого примера уволился и больше не нужно хранить день его рождения:
...
- `pop()` -- метод, который достает значение, хранящееся по переданному ключу, и удаляет ключ из словаря:
...
Для метода `pop()` можно указать значение, которое будет возвращено при обращении к несуществующему ключу. Почти как `get()`, но без указания этого значения, `pop()` выбрасывает `KeyError`.

- `popitem()` имеет схожее название, но в отличие от предыдущего метода на вход он не принимает `ключ`, а возвращает пару `ключ`: `значение`, которая была добавлена последней. Такое поведение гарантируется с Python 3.7.
...
print(len(holidays))
```
Обратите внимание на то, как работают методы `pop()`, `popitem()` и `clear()`: как только они вызываются, словарь меняет свой состав. Изменения происходят in place, то есть "на месте" без копирования.

### Обновление и добавление ключей
Мы уже видели, что значения в словарь можно добавлять или менять, обращаясь по ключу. Python предоставляет возможность не писать кучу присваиваний, а использовать лаконичный метод `update()`, который на вход может принимать:
- другой словарь;
- пары `ключ`: `значение` в какой-то последовательности (например, кортежи по два значения в списке: первое -- ключ, второе -- значение).

Кортеж (tuple) -- еще одна стандартная структура данных. Очень похожа на list и поддерживает индексацию, но является неизменяемой.
...
В Python можно извлекать отдельно по ключам или значениям, а также итерироваться по элементам словарей в цикле `for`. 
...
переменные цикла (тут -- `key`) будут принимать значения из множества ключей словаря. Аналогично можно использовать метод `keys()` -- он позволяет достать все ключи. Метод явно говорит, что цикл идет по ключам, например:
...
### Все и сразу
Существует метод `items()`, который достает пары `ключ`: `значение` в виде последовательности кортежей. Его удобно использовать в циклах, чтобы не тащить длинную запись в виде названия словаря и квадратных скобок с ключом при обращении к значению:
...
## Сортировка
Функция `sorted()` доступна для словарей. По умолчанию ключи словаря поддерживают порядок, в котором были добавлены, но можно отсортировать их в нужном направлении (_в зависимости от типа_):
```{code-cell} ipython3
...
Когда в функции передается просто название переменной со словарем, работа идет только над множеством ключей.

Можно ли отсортировать словарь по значениям? Да, можно попробовать самостоятельно разобраться с [аргументами функции `sorted()` в документации по Python](https://docs.python.org/3/howto/sorting.html).

## Что можно хранить
`Dict` может принимать в качестве ключа не всякое значение. Только [хэшируемые](https://docs-python.ru/tutorial/vstroennye-funktsii-interpretatora-python/funktsija-hash/) объекты _(можно вызвать функцию `hash()` и получить значение)_ могут быть ключами словаря, на значения это ограничение не распространяется. В `dict` и `set` значение хэша от объекта используется для поиска внутри структуры.
...
_В англоязычной литературе изменяемые типы называют mutable, а неизменяемые -- immutable, подробнее -- в [соответствующем разделе документации по Python](https://docs.python.org/3/reference/datamodel.html#objects-values-and-types)_

В Python все -- объект. Когда пользователь присваивает значение переменной, она начинает ассоциироваться с ячейкой памяти, где лежит это значение. Переменная знает адрес, откуда можно получить значение. `id()` и `hex()` показывают адрес в памяти компьютера. _`id()` - адрес в десятичном виде, а `hex()` поможет перевести его в _шестнадцатеричный_.
...
- изменяемые типы позволяют менять внутреннее состояние переменной при сохранении адреса (возвращаемое `id()` значение не меняется, но значение переменной преобразовывается). Изменение по ссылке называется изменением in place.
...
# полученное значение адреса может отличаться
...
```
Кажется, что раз значение переменной `counter` поменялось, то и содержимое по предыдущему адресу изменилось. На самом деле `counter` теперь указывает в другое место:
...
После добавления еще одного элемента адрес `ratings` не изменился.
...
- Есть несколько способов создания `dict`, например:
...
- Есть несколько методов изменения состояния или получения доступа к элементам:
...
- Есть изменяемые (`dict`, `set`, `list`) и неизменяемые (`int`, `float`, `bool`, `tuple`, `str`) стандартные типы данных.
