...
Определим fidelity между тестовым состоянием и $j$-тым тренировочным $|\phi_j\rangle$ определим как
...
4. Применяем swap test между тренировочным регистром $tr$ и тестовым регистом $tst$, а регистр $B$ будет выступать в качестве контрольного.
    ...
    Определим $U$  как унитарное преобразование, которое объединяет шаги 3 -- 4. Кстати, если мы сейчас произведём измерение регистра $B$, то будем иметь сле
    ...
    На этом шаге информация о fidelity закодирована в амплитуды. Теперь же мы должны перевести fidelity из амплитуды в число.
...
14. О да... Теперь мы дообавляем ещё один кубит $Q_3$. Применяем гейт $X$ на кубите $Q_2$ и гейт Тоффоли с контролирующими $(Q_1, Q_2)$ и целевой $Q_3$
