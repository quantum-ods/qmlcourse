...
А $F = [F_0, F_1, ..., F_{M-1}]$ - это таблица $fidelity$ значений между тестовым сотоянием и каждым из тренировочных.
...
Тогда квантовый алгоритм поиска $k$ ближайших соседей будет состоять из двух основных шагов:
...
2.  Берём 2 пары регистров $i_1, f_1; i_2, f_2$ (index_1, fidelity_1; index_2, fidelity_2). Инициализируются они в форме $|j\rangle_{i_1}|0\rangle_{f_1}|y\rangle_{i_2}|0\rangle_{f_2}$
...
13. Добавляем ещё один кубит $Q_2$ для каждого $i_l \in A$, применяя гейт $D^{(i_l)}$
    ...
    $$
      |j\rangle_{i1}|F_j\rangle_{f1}|g(j)\rangle_{Q_1}|\khi_A(j)\rangle_{Q_2}
    $$
...
14. О да... Теперь мы дообавляем ещё один кубит $Q_3$. Применяем гейт $X$ на кубите $Q_2$ и гейт Тоффоли с контролирующими $(Q_1, Q_2)$ и целевой $Q_3$
    ...
    $$
      |j\rangle_{i1}|F_j\rangle_{f1}|g(j)\rangle_{Q_1}|\khi_A(j)\rangle_{Q_2}|f_{y,A}(j)\rangle_{Q_3}
    $$
