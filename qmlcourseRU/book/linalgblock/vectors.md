...
Использую эти две операции, мы можем считать **линейные комбинации векторов**:
$$
\alpha_1 \mathbf{a}_1 + \alpha_2 \mathbf{a}_2 + ... + \alpha_n \mathbf{a}_n ,
$$

где $\mathbf{a}_1, \mathbf{a}_2, ..., \mathbf{a}_n$ - векторы, а $\alpha_1, \alpha_2, ..., \alpha_n$ - числа.
...
С помощью данной формулы можно вывести одно из главных свойств скалярного произведения, которое заключается в том, что **два вектора перпендикулярны тогда и только тогда, как их скалярное произведение равно 0**:
$$
...
Откуда же появляются его количественные измерения? Для того, чтобы это стало понятно, нужно ввести понятия **базиса**.

Базис - это конечноый набор векторов в векторном пространстве, такой, что любой вектор этого пространства может быть единственным образом представлен в виде линейной комбинации векторов из этого набора.

Вспомним один из примеров выше, где мы представляли прямоугольную систему координат на плоскости и единичные векторы (орты) $\mathbf{e}_1$, $\mathbf{e}_2$.
$$
...
**Любые $n$ линейно независимых векторов $n$-мерного пространства образуют базис этого пространства.**

Именно из-за линейной зависимости векторов $\begin{pmatrix} -2 \\ 0 \end{pmatrix} \begin{pmatrix} 0 \\ -3 \end{pmatrix}$ они не могут являтся базисом двумерного пространства.
