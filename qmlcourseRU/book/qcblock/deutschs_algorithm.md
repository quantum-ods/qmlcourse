...
Здесь ситуация сложнее, так как $f(x)$ будет зависеть от состояния первого кубита. Поэтому мы раскроем скобки, а ее значения подставим позже:
...
Получили то же состояние $|\psi\rangle$, что и для $f_3$ с точностью до фазового множителя. Соответственно, здесь первый кубит в итоге также будет измерен с результатом $1$.
