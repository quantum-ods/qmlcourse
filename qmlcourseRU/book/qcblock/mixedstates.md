...
```{note}
Вспомним обсуждение кота Шредингера в [лекции про кубит](qubit) -- упрощая до предела, можно считать, что радиоактивный атом является одной системой, а несчастный кот -- второй.
...
В таком представлении можно сказать, что смешанное состояние -- это ситуация, когда мы не можем сопоставить квантовой системе никакой конкретный вектор, а считаем, что существует статистическая смесь из нескольких возможных состояний $\ket{\phi_i}$, каждое со своей вероятностью $d_i$.
...
Квантовая механика работает так, что любое [измерение](../qcblock/qubit.html#id23) приводит к коллапсу волновой функции и является необратимым. А еще измерения, например, состояния $\ket{+}$ и $\ket{-}$ не различимы при измерениях по оси $\mathbf{Z}$ -- для обоих состояний мы будем получать $\ket{1}$ и $\ket{0}$ с вероятностью $0.5$, то есть нам нужно измерять по _всем_ базисам. В общем, получается, что восстановить амплитуду и фазу волновой функции $\Psi$ это большая проблема, если добавить сюда вероятностный характер измерения.
...
В квантовой механике существует также численная характеристика того, насколько сильно наша система "запутанная". Эта величина называется энтропией фон Неймана, а ее выражение очень похоже на выражение для классической [энтропии Шеннона](https://ru.wikipedia.org/wiki/Информационная_энтропия). Только в отличие от классики, в квантовой механике мы имеем _матрицу_ плотности, поэтому в выражении у нас фигурирует [матричный логарифм](https://en.wikipedia.org/wiki/Logarithm_of_a_matrix):
...
Энтропия фон Неймана -- одно из ключевых понятий в квантовой теории, которое играет важную роль, например, в изучении черных дыр.
...
```{code-cell} ipython3
def entropy(rho: np.array, eps: float = 1e-16) -> float:
    e = -np.sum(np.diag(rho) * np.log(np.diag(rho) + eps))
    return np.real(e)
```

```{note}
Тут мы в очередной раз пользуемся функциями из библиотеки линейной алгебры `SciPy`. Документацию по функции `scipy.linalg.logm` можно посмотреть на [официальном сайте](https://docs.scipy.org/doc/scipy/reference/generated/scipy.linalg.logm.html).
```
