...
Для начала мы должны подготовить наши регистры по всем квантовым законам: $|b\rangle$ и $|x\rangle$ должны быть пронормированы, а оператор $A$ должен быть эрмитовым (напомню, что эрмитов оператор при транспонировании равен комплексно сопряжённому – найдите, перечитайте и ещё раз осмыслите этот факт, он нам нужен). Надеемся, что Вы помните про ортонормированный базис, сферу Блоха, комплексное представление векторов $|0\rangle$ и $|1\rangle$... Если нет, то обратитесь к предыдущим разделам курса.

Мы будем использовать оператор $U = e^{iAt}$, и нужно, чтобы он был обратим – для этого $А$ должна быть эрмитовой.

Но прежде мы должны воспользоваться одним важным свойством: эрмитову матрицу $А$ представим в виде суммы собственных векторов, умноженных на собственные значения, т.е. в виде спектрального разложения:
...
$$
\large  QPE(|0\rangle|\psi\rangle) = 
$$
$$
\large\frac{1}{2^{m/2}}( |0\rangle + e^{2\pi i 2^{m-1} \psi} |1\rangle) \otimes (|0\rangle + e^{2\pi i 2^{m-2} \psi} |1\rangle) \otimes \dots \otimes  (|0\rangle + e^{2\pi i 2^{0} \psi} |1\rangle) \otimes |\psi\rangle = 
$$
Алгоритм обратного квантового Фурье переводит фазу в конкретный вектор.


$$
\large \frac{1}{2^{m/2}}\sum_{l=0}^{2^{m-1}} e^{2\pi i \psi l} |l\rangle
$$
...
Шаг 1. Применение QPE с использованием преобразования $e^{iA}$:

$$
|0\rangle_{a}|0\rangle_{r}|b\rangle_{m} \rightarrow \sum_{j=0}^{N-1}\beta_j|0\rangle_{a}|\lambda_j\rangle_r|u_j\rangle_m
$$
...
Шаг 2. Дело осталось за малым. Нам нужно всего-то перевернуть все лямбды и вернуть им вероятностную, а не векторную форму. За это отвечает специальный оператор вращения $R$:

$$
R|0\rangle = \sum_{j=0}^{N-1}\left(\sqrt{1-\frac{c^{2}}{\lambda_{j}^{2}}}|0\rangle + \frac{c}{\lambda_{j}}|1\rangle\right),
$$
...
$$
\sum_{i=0}^{N-1}\beta_i|0\rangle_{a}|\lambda_j\rangle_r|u_j\rangle_m \rightarrow
$$
$$
...
Шаг 3. Применяем обратное QPE и получаем состояние будут следующими:
$$
\sum_{j=0}^{N-1}\left(\sqrt{1-\frac{c^{r}}{\lambda_{j}^{2}}}|0\rangle+\frac{c}{\lambda_j}|1\rangle\right) b_{j}|0\rangle_{n}\left|u_{j}\right\rangle_{m}
$$
