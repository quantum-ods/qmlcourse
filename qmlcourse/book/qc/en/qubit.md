...
This lecture tell about:
- what is qubit;
- what's the difference between value and state;
- what is the Bloch sphere;
- which operations could be done with qubit;
- what is the measurement.


## Introduction

This is the first lecture of the main block of the course. Before we deep into details about qubit let's look at the generic pipeline of a typical quantum circuit:


```{figure} /_static/qc/ru/qubit/any_qubit_diagram.svg
:name: qscheme

Generic scheme of quantum algorithm
```

Almost every quantum circuit contains:

- qubits, initialized in the initial state (typically it is $\ket{0}$);
- unitary and reversible operations with qubits;
- measurements.

This lecture is about deep dive into operations with a **single** qubit. We will start from the definition and meaning of qubit and the difference between quantum and classical computations.

## What is the qubit

A classical computer works with binary number: zero and one. A minimal amount of information for a classical computer is named **bit**. Quantum computer works with a quantum version of bits or **qubits** which have two unique values (0 and 1) too. In this case what is the difference? What special is in these qubits and quantum computers that give them such an advantage compared to classical computers?

The difference is that in quantum systems (system of qubits is a quantum system) _state_ and _value_ is not the same.

## State vs Value

### A state of classical bit

Typically we do not distinguish between a state of a classical bit and it's value and declare that in the case when a bit has value **1** it's state can be described with a number **1**.

### Schrodinger's cat

Let's remember the Schrodinger's mind experiment. A Cat which is both dead and alive in the same moment. Obviously the _value_ of such a Cat should be defined exactly: he can be alive or dead not both. But a _state_ of this Cat is a more complex thing. He is in _superposition_ of states "alive" and "dead" in the same moment.
...
A state of qubit is an analogue to the state of a Schrodinger's Cat. Generally speaking it is different from the _value_ of qubit and described by a vector of two complex numbers. We will use a symbol $\ket{\Psi}$ ($\ket$ is a column-vector) do describe a state or a state-vector of the qubit. It is a widely used in quantum computations and quantum mechanics Dirac's notation:
...
```{note}
There may be a question why should we use complex numbers? And there is no short answer. Long story short using complex numbers simplify a definition of matrix groups which are used in quantum mechanics.


Is it still unclear? Well in this case we should remember that quantum mechanics had been created to explain `wave-particle duality` which was experimentally proved by that moment. In other words electrons, photons and other particles shown both signs of wave moution (experimentally it was proved by diffraction and interferention) and signs of classical particles like a minimal amount (`quant`!) of light or electrical field. Offen instead of using definition of state vector one uses a `wave function` which describe the probability density of chance to find the particle in one or another point of space (regular one like coordinates space or special one like space of energies). By the moment of invention of quantum mechanics to explain wave motion of particles physics already used to use complex numbers which are simplifying description of a lot of effects because complex numbers allows you to divide amplitude and phase. Such a simplification is useful in quantum physics too.

More detailed answer can be found in books about history of physics (and books about qunatum mechanics).
```

Values of numbers $c_0$ и $c_1$ will be explained little later. Now let's define our qubit $\ket{\Psi}$ as a Python code. At first with values $c_0 = c_1 = \frac{1}{\sqrt{2}}$.
...
Here we create column-vector with shape $2 \times 1$.
...
### Relation between sate and value of the qubit
